...
import { treeContainerWith } from '../core/tree';
...
function overlayerMouseup(evt) {
  ...
  // TODOTREE
  this.dragFlag = false;
  this.tree.mouseupEvt(ri, ci, top, height, width, offsetX, offsetY);
}
...
// TODOTREE 给树添加层级level
function addLevel() {
  const { data } = this;
  console.log('data', data)
  // data.addLevel()
  // const { selector, rows, tree } = data;
  // const { ri } = selector;
  ...
  // this.tree.resetData(this.data)
  this.data.addLevel()
}

// TODOTREE
function hideRowsOrCols(data) {
  this.data.hideRowsOrCols(data);
  // console.log('data', data)
  sheetReset.call(this);
}
...
function overlayerMousedown(evt) {
  ...
  let { ri, ci } = cellRect;
  // sort or filter  // TODOTREE
  const { autoFilter, settings } = data;
  ...
  // TODOTREE
  this.tree.clickEvt(ri, ci, top, height, width, offsetX, offsetY);
  ...
  if (!evt.shiftKey) {
    ...
    if (isAutofillEl) {
      ...
      // TODOTREE
      if(offsetX > treeW) selectorSet.call(this, false, ri, ci);
    }
    ...
    mouseMoveUp(window, (e) => {
      ...
      if (isAutofillEl) {
        ...
        // TODOTREE
        if(offsetX > treeW) selectorSet.call(this, true, ri, ci, true, true);
      }
    }, () => {
  ...
  if (!isAutofillEl && evt.buttons === 1) {
    if (evt.shiftKey) {
      ...
      // TODOTREE
      if(offsetX > treeW) selectorSet.call(this, true, ri, ci);
    }
  }
}
...
function toolbarChange(type, value) {
  ...
  if (type === 'undo') {
    ...
    // 自定义工具栏的回调
    if(data.settings.defineToolbar) {
      data.settings.defineToolbar.forEach(it => {
        if(Array.isArray(it)) {
          it.forEach(child => {
            if (type === child.trigger) {
              this.trigger(child.trigger, this.data)
            }
          })
        }
      })
...
function sheetInitEvents() {
  ...
  // contextmenu
  contextMenu.itemClick = (type, trigger) => {
    ...
    if (type === 'validation') {
      ...
      if(trigger) {
        ...
        if (trigger === 'level') {
          ...
          this.trigger(trigger, this.data.selector, cell);
        }
      }
...
export default class Sheet {
  constructor(targetEl, data) {
    ...
    const { view, showToolbar, showContextmenu,defaultContextMenu, defineContentMenu, defaultToolbar, defineToolbar } = data.settings;
    this.el = h('div', `${cssPrefix}-sheet`);
    this.toolbar = new Toolbar(data, view.width, !showToolbar, defaultToolbar, defineToolbar);
    ...
    // contextMenu
    this.contextMenu = new ContextMenu(() => this.getRect(), !showContextmenu, defaultContextMenu, defineContentMenu);
    // selector
    ...
    // TODOTREE
    this.treeEl = h('div', 'dt-tree');
    this.dragFlag = false; 

    // root element // TOODOTREE
    this.el.children(
      this.tableEl,
      this.overlayerEl.el,
      this.rowResizer.el,
    ...
    let cb = function(res) {
      // console.log('res', res)
      hideRowsOrCols.call(self, res)
    }

    this.tree = new Tree(this.treeEl.el, data, cb);

    // table // TODOTREE
    this.table = new Table(this.tableEl.el, data,  this.tree);
  ...
  getTableOffset() {
    // TODOTREE  添加一个settings
    const { rows, cols, settings } = this.data;
    const { width, height } = this.getRect();
    // TODOTREE 设置tree的宽度
    const treeW = treeContainerWith(settings);
    return {
      width: width - cols.indexWidth - treeW,
      height: height - rows.height,
      left: cols.indexWidth + treeW,
      top: rows.height,
    };
  }
}
