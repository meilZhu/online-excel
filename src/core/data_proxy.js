...
//  TODOTREE
import { treeContainerWith } from './tree';
...
function getCellColByX(x, scrollOffsetx) {
  const { cols, settings } = this;
  const fsw = this.freezeTotalWidth();

  //TODOTREE 初始化时候顶部离左侧的距离
  let inits = cols.indexWidth + treeContainerWith(settings);
...
export default class DataProxy {
  constructor(name, settings) {
    ...
    // save data begin TODOBOTTOMBAR
    this.name = name || 'sheet';
    this.bottombarHeight = this.settings.showBottomBar ? bottombarHeight : 10;
    this.freeze = [0, 0];
  ...
  // type: row | column  // TODOTREE 添加一行时，对数的处理
  insert(type, n = 1) {
    this.changeData(() => {
      ...
      if (type === 'row') {
        ...
        // TODOTREE
        if (!this.tree) return;
        this.tree.data.splice(sri, 0, 
          {expand: true, level: sri < this.tree.data.length ? this.tree.data[sri].level : 0, ri: sri, childrenLen: 0, parent: sri < this.tree.data.length ? this.tree.data[sri].parent: '' }
          )
        this.addParentLen(this.tree.data[sri].parent ? this.tree.data[sri].parent.id : '', 1)
        this.tree.data.forEach( (it, index) => {
          it.ri = index
        })
      } else if (type === 'column') {
  ...
  // 将tree的新增列的父级的childrenLen + 1
  addParentLen(parentId, len) {
    if (!parentId) return;
    this.tree.data.forEach( it => {
      if (it.id === parentId) {
        it.childrenLen = it.childrenLen + len;
        this.addParentLen(it.parent ? it.parent.id: '', len)
      }
    })
  }
  ...
  addLevel() {
    this.changeData(() => {
      ...
      const { ri } = selector;
      if (tree.data[ri].childrenLen) return;

      // 在右侧添加一列
      rows.insert(ri + 1, 1)
      // 左侧处理
      if (tree.data[ri].lastChild) {
      ...
      tree.maxLength = maxLevel + 1;
      merges.shift('row', ri, 1, (ri, ci, rn, cn) => {
        const cell = rows.getCell(ri, ci);
        cell.merge[0] += rn;
        cell.merge[1] += cn;
      });
  ...
  delete(type) {
    this.changeData(() => {
      ...
      if (type === 'row') {
        ...
        const len = eri - sri + 1;
        this.tree.data[sri-1].lastChild = (this.tree.data[eri].lastChild &&  this.tree.data[sri-1].childrenLen !== 1 && this.tree.data[eri].level === this.tree.data[sri-1].level) || this.tree.data[sri-1].lastChild ? true : false;
        this.deleteParentLen(this.tree.data[sri].parent ? this.tree.data[sri].parent.id: '', len);
        this.tree.data.splice(sri, len);
        this.tree.data.forEach( (it, index) => {
          it.ri = index
        })
   ...
   // 将tree的删除列的父级的childrenLen - 1
  deleteParentLen(parentId, len) {
    if (!parentId) return;
    this.tree.data.forEach( it => {
      if (it.id === parentId) {
        it.childrenLen -= len;
        this.deleteParentLen(it.parent ? it.parent.id: '', len)
      }
    })
  }
  ...
  hideRowsOrCols(data) {
    ...
    const treeData = this.tree.data ? this.tree.data : [];
    ...
    // TODOTREE
    if(!data) {
      console.log(sri, sci, eri, eci, rlen, clen, '----')
      if (rlen === rows.len) {
        for (let ci = sci; ci <= eci; ci += 1) {
  ...
  exchangeLevel(item) {
    ...
    this.tree.data[ri].expand = !this.tree.data[ri].expand;
    // 重绘左侧的tree
    this.change(this)
  }
  ...
  hideLevelTree(num, level, item) {
    ...
    if (!item.hideLevel) {
      ...
      rows.setHide(num, true)
    }
  }


  // TODOTREE 树形结构的拖拽事件
  dragTreeData(oldItem, newItem) {
    if (oldItem.ri === newItem.ri) return;
    this.changeData(() => {
      const { rows } = this;
      let sumLevel = 0;
      const oldIndex = oldItem.ri; // 按下时的索引
      const oldLevel = oldItem.level;  // 鼠标按下时的所在行的层级level
      const len = 1 + oldItem.childrenLen; // 拖抓走的长度
      const sliceTree = this.tree.data.slice(oldIndex, oldIndex + len)  // 拖拽走的数据
      const newIndex = newItem.ri;  // 放开鼠标时的索引
      const newLevel = newItem.level;  // 鼠标放开时的层级level、
      const level = newLevel - oldLevel; // 按下鼠标和松开鼠标时层级level的差
      const sumArr = []
    
      // 先作删除处理
      this.tree.data.splice(oldIndex, len);
      // 对一个层级最后一个的处理
      if (oldItem.lastChild && oldItem.level === this.tree.data[oldIndex - 1].level) {
        this.tree.data[oldIndex - 1].lastChild = true;
      }
      this.deleteParentLen(oldItem.parent ? oldItem.parent.id : '', len)

      // 这里是对左侧树形结构数据的处理 (这里是添加)
      // 从上方往下放移动
      if (oldIndex < newIndex) {
        sliceTree.forEach((it, i) => {
          ...
          this.tree.data.splice(newIndex + i - len, 0, it);
        })
      // 从下方向上方移动
      } else {
        sliceTree.forEach((it, i) => {
          ...
          this.tree.data.splice(newIndex + i, 0, it);
        })
      }
      
      this.addParentLen(newItem.parent ? newItem.parent.id : '', len)


      this.tree.data.forEach( (item, idx) => {
        item.ri = idx
        if (item.level > sumLevel) {
          sumLevel = item.level
        }
      })
      this.tree.maxLength = sumLevel + 1;
      // 这里需要重绘tree
      this.change(this)

      // 这里是对右侧数据的处理
      for (let i = 0; i <len; i++) {
        sumArr.push(rows._[oldIndex + i])
      }
      rows.insert(newIndex, len)
      sumArr.forEach( (t,indx) => {
        rows._[newIndex + indx] = t
      })

      if (oldIndex < newIndex) {
        rows.delete(oldIndex, oldIndex + len - 1)
      } else {
        rows.delete(oldIndex + len, oldIndex + len + len -1)
      }
    })
